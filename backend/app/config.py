...
class Settings:
    ...
    #
    # Supported:
    #   demo  -> local simulated data
    #   live  -> RailRadar
    #   ntes  -> NTES-based live provider
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # GENERIC RAILWAY PROVIDER CONFIGURATION
    # ---------------------------------------------------------

    railway_api_base_url: str = _env(
        "RAILWAY_API_BASE_URL",
    )

    railway_api_key: str = _env(
        "RAILWAY_API_KEY",
    )
    ...
    railradar_api_base_url: str = _env(
    ...
    weather_api_base_url: str = _env(
        "WEATHER_API_BASE_URL",
    )

    weather_api_key: str = _env(
        "WEATHER_API_KEY",
    )

    openweather_api_key: str = _env(
        "OPENWEATHER_API_KEY",
    )
    ...
    realtime_interval_seconds: float = float(
        _env(
    ...
    def validate(self) -> "Settings":
        ...
        if self.data_provider not in {
            "demo",
            "live",
            "ntes",
        }:
            raise ValueError(
                "DATA_PROVIDER must be "
                "'demo', 'live' or 'ntes'"
            )
