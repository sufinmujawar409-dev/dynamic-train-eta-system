...
# DEMO is accepted here because the realtime layer may use
# "DEMO" while the lower-level simulation uses "SIMULATED".
DataQuality = Literal[
    "DEMO",
    "SIMULATED",
    "LIVE",
...
class WeatherData(BaseModel):
    ...
    weather_delay_risk: WeatherRisk = (
        "UNAVAILABLE"
    )
...
class Train(BaseModel):
    ...
    weather: WeatherData = Field(
        default_factory=WeatherData.unavailable
    )
...
class TrainPosition(BaseModel):
    ...
    distance_to_next_station: float | None = Field(
        default=None,
        ge=0,
    )
...
class TrainTelemetry(BaseModel):
    """
    Validated source snapshot before ETA
    ...
    distance_to_next_station: float = Field(
        default=0,
        ge=0,
    )
    ...
    route: list[dict[str, object]] = Field(
    ...
    @classmethod
    def train_id_must_not_be_empty(
        cls,
        value: str,
    ) -> str:
...
# =========================================================
# REALTIME TRAIN EVENT
# =========================================================

class RealtimeTrainEvent(BaseModel):
    """
    Stable WebSocket contract consumed
    by the dashboard.
    """
